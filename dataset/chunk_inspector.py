...
def inspect_from_json(max_docs: int = 5):
    ...
    for i, doc in enumerate(data[:max_docs]):
        # 실시간 청킹 수행
        stats = _chunker.chunk_with_stats(doc['text'])

        print(f"\n📄 문서 #{i+1} | 카테고리: {doc['category']} | 날짜: {doc['law_date']}")
        print(f"   전체 글자 수: {len(doc['text'])}자 | 청크 수: {stats['total_chunks']}개")
        print("-" * 70)

        for chunk in stats["chunks"]:
            # 값이 비어있을 경우를 대비해 '미추출' 처리
            law_name_str = chunk.law_name if chunk.law_name else "미추출"
            article_str = chunk.article if chunk.article else "미추출"
            
            print(f"  [청크 {chunk.chunk_index}] ({chunk.char_count}자) | ⚖️ 법령명: {law_name_str} | 📑 조: {article_str}")
...
if __name__ == "__main__":
    ...
    政治的·經濟적·社會적·文化的 생활의 모든 領域에 있어서 差別을 받지 아니한다.
